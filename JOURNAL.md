...
**설계 결정 — 포장대기게이트는 순수 워처.** 처음엔 게이트2가 스스로 "지연 아이템 도착"을 흉내 내고
...
필요한데, 그래프 재진입성 자체가 이미 DESIGN.md 5단계 진행상황에 "다음 세션 과제"로 못박혀
...
인정하기로 함 — 억지로 다른 트리거(예: 서로 다른 배송지)로 흉내 내면 "주소가 다르니 당연히
...
고치면 포장대기게이트 쪽에 "escalated 없이 영원히 미봉인"이라는 사각지대가 남는다 — 예를 들어
"계속대기희망"으로 결정된 item을 패키지가 영원히 기다리다 재시도 예산이 소진되는 경우(과거
시나리오6의 연쇄 패턴과 동일한 모양)가 그렇다. 두 게이트 모두 같은 보상조치(환불) 모델로
수렴시켜(`_compensate` 헬퍼 공유) 이 사각지대를 없앴다. DESIGN.md 초안엔 명시 안 됐던
부분이라 이번에 코드 작업 중 도출.
