...
6. **`derive_internal_order_status`가 패키지 하나라도 미봉인이면 order 전체를 "조립중"으로
   ...
   **(코드 리뷰 후속: 위 "5단계 후속" 판단은 근거가 부정확했다.** v16의 "부분완료"/"전체무산"은
   `_permanently_excluded`로 걸러지는 케이스(품목취소·부분수령 영구제외)를 위한 것이지,
   "합배송희망으로 계속 대기 중인 미봉인 패키지" 문제와는 무관하다 — 그런 item은 취소되지도
   부분수령 제외되지도 않아 `_permanently_excluded`가 걸러내지 못한다. 이 gap이 실제로
   재현되지 않았던 진짜 이유는 별개 메커니즘이었다: `packaging_wait_gate`가 "주문에 속한
   패키지 중 하나라도 미봉인이면 전부 재시도"라는 order-wide 블로킹으로 걸려있어서, 어떤
   패키지든 배송중게이트~추적agent 루프에 진입하는 시점엔 이미 미봉인 패키지가 하나도
   없다는 게 구조적으로 보장된다 — `derive_internal_order_status`의 조기 반환 분기가
   tracking_agent 안에서는 사실상 죽은 조건이 되는 것. 즉 "해소됐다"가 아니라 "다른
   블로킹에 우연히 가려져 있었다"가 정확한 설명이다. 이 packaging_wait_gate 쪽 블로킹
   자체는 배송중게이트와 달리 지금도 고쳐지지 않은 채 남아있다 — DESIGN.md "아직 결정
   안 된 것" 참고.)**
...
이전 세션에서 RPM 6 / RPD 27 한도로 실행이 중단된 적이 있었다. 정확한 에러 메시지는 세션
경계를 넘기며 소실됐다 — 재현 어려운 외부 API 에러는 발생 즉시 기록해야 한다는 교훈.

**대응 1 — 모델 교체**: 이 프로젝트가 Gemini에 요구하는 작업(구조화 출력 하나, 입력 신호
4개, 판단 1~2문장)은 단순 분류 수준이라 flash-lite급으로 충분하고, 가벼운 모델일수록
무료 티어 RPM/RPD 여유가 나은 경향이 있어 교체했다(정확한 한도는 AI Studio 대시보드에서
계정별 확인 필요). 시나리오10을 신/구 모델로 각각 격리 실행해 판단/근거 품질이 동등함을
확인했다.

**대응 2 — RPM 안전장치**: `predict_delay_escalation` 호출 직전에 최소 호출 간격
(`GEMINI_RPM_LIMIT`, 기본 6회/분)을 강제하는 스로틀을 추가했다. 모델별 실제 한도를 조회해
정교하게 맞추기보다 보수적 고정값을 쓴 이유는 "재발 방지"가 목적이지 처리량 최적화가
아니기 때문 — env var라 실제 한도 확인 후 조정 가능하다. 연속 호출 테스트로 간격이 정확히
10초로 맞춰지는 것과, 실제 데모 흐름(시나리오5→10 사이)에서도 자연 발동하는 것을 확인했다.
