...
**파생 이슈2: `PackageState.escalated`가 조립대기게이트/배송중게이트 공유 필드였다.** DESIGN.md
5단계 초안은 보상조치 재설계를 배송중게이트(회복불가/회복가능 분류) 관점에서만 다뤘는데,
`escalated` 필드 자체는 조립대기게이트도 (재시도 소진 시) 세팅하는 공유 필드다. 배송중게이트만
고치면 조립대기게이트 쪽에 "escalated 없이 영원히 미봉인"이라는 사각지대가 남는다 — 예를 들어
