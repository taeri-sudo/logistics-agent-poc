...
- **`escalated`** — 舊 State 필드명(v16에서 제거). "재시도 한도를 넘었다/조기 신호가 있었다"는
  **현재 상태**를 기록하는 값일 뿐, 그 자체로는 행위가 아니었다.
...
1. **결정화된 규칙** (재고부족/검수불량 등) — 해소 시점이 경험적으로 고정 가능한 예외.
   ...
   `_ITEM_RESOLVE_AT_RETRY`의 재고부족(`resolve_at=2`)/검수불량(`resolve_at=1`)도 아래
   자연재해와 같은 성격의 데모 단순화다 — 재고가 실제로 언제 채워지는지는 실 서비스라면
   재고센서/외부 API 폴링 결과로 알아야 하는, 도메인적으로는 교통지연과 마찬가지로 해소 시점이
   불확실한 사유다. 지금 코드가 이를 "결정화된 규칙"(계층1)으로 분류한 근거는 도메인 성질이
   아니라, 이 POC가 재고부족/검수불량을 신호 기반이 아니라 고정 재시도 횟수로 mock했기
   때문이다(자연재해가 신호를 미리 알고 스크립트를 짠 것과 동일한 종류의 단순화 — 아래
   자연재해 설명 참고). 실제 서비스 전환 시에는 재고부족도 계층2(예측/신호 기반)로 재분류될
   가능성이 있다.
...
| 판단+반복 | 피킹지연게이트 | self-loop 조건분기 + Stage1 자동판정 | Item 기반. `item_delay_reason` 있는 item만 대상, 해소되면 피킹완료 확정, 미해소면 자기루프. retry_count 초과 시 Stage1 판정(`_ITEM_RECOVERABLE`)으로 즉시 귀결 — 회복불가(파손)면 품목취소, 회복가능(재고부족/검수불량/통관지연)이면 `fulfillment_preference_on_delay`로 부분수령/합배송 자동 적용 (v16) |
...
- ~~split_delivery_preference (bool)~~ → 제거 (v16). "사전에 무조건 분리"라는 선택지 자체가
  잘못 설계였다는 판단 — 실제 분리 필요는 `fulfillment_preference_on_delay`의 "부분수령희망"이
...
> `fulfillment_preference_on_delay_hint` / `order_created_at_hint`(진입 입력),
...
| fulfillment_preference_on_delay | string(enum)/null | 부분수령희망/합배송희망/null(=합배송희망과 동일 취급). 주문 시점 사전등록, 문제가 실제 발생했을 때만 참조됨(v16, 舊 split_delivery_preference) |
...
|---|---|---|
| item_id | string | |
| item_status | string(enum) | 대기/피킹중/피킹완료/포장완료/출고됨/배송중/배송지연/배송완료/취소됨 |
| item_delay_reason | string/null | 재고부족/검수불량/파손/통관지연 등 Item 고유 지연 원인. 값이 있으면 창고처리agent가 피킹 스킵. 취소 확정 후에도 null로 안 되돌림(v16) |
| package_ref | string/null | 소속 Package (조립 전 null, 취소/부분수령 제외 item은 영구 null) |
...
| customer_facing_status | string(enum) | 파생값. item_status를 사용자용으로 매핑. item_delay_reason이 있는 동안(재시도 중)은 피킹지연게이트가 "지연"으로 덮어쓰고, 취소 확정 시 "상품준비불가"로 덮어씀(v16) |
...
- ~~재고부족이 영구 에스컬레이션되면 customer_facing_status가 "지연"인 게 맞는지~~ →
  v16에서 해소됨: 회복불가 사유(파손)는 즉시 품목취소+"상품준비불가"로, 재고부족/검수불량/
  통관지연(회복가능)은 `fulfillment_preference_on_delay`로 자동 처리된다.
- **그래프 재진입성 미구현.** `fulfillment_preference_on_delay="부분수령희망"`으로 그룹핑에서
...
- **봉인 후 보상조치(환불)된 item의 `customer_facing_status`와 `internal_order_status`가 둘 다
  환불 사실을 전혀 드러내지 않는다.** 근본 원인은 하나다 — `mock_carrier_signal`이 보상조치된
  패키지도 계속 전진시킨다는 "비차단" 원칙에 따라 물리적으로는 배송 시퀀스를 그대로 밟아
  "배송완료"까지 도달시키고(`compensation`이 세팅돼도 `delay_categories`는 지워지지 않는데,
  `_permanently_excluded`는 "봉인된 채 보상조치"는 영구제외로 치지 않는다 — 미봉인 상태로
  보상조치된 경우만 다루도록 설계돼 있다), 그 결과를 `tracking_agent`가 그대로 반영해버린다.
  - `customer_facing_status`: item_status가 "배송완료"에 도달하는 순간 "지연"을 덮어쓰고
    "배송완료"로 표시된다(시나리오 7로 확인) — 고객은 환불받았다는 사실을 어디서도 알 수 없다.
  - `internal_order_status`: 봉인된 채 보상조치된 item은 `_permanently_excluded`에서 걸러지지
    않아 계속 "shippable"로 취급되고, 결국 `derive_internal_order_status`가 "완료"(제외된 item이
    함께 있으면 "부분완료")로 판정한다(시나리오 7 재실행, 2026-08-30 실측 확인 — 자연재해로
    즉시 환불된 단일 패키지 주문이 `internal_order_status="완료"`로 나옴). 舊 "미봉인 상태로
    보상조치되면 조립중에 영원히 멈추는" 버그(위 진행 상황 6단계 후속)와 달리 이건 그래프가
    정상 종료하고 파생값도 유효한 종결값에 수렴하므로 즉시 고쳐야 할 결함은 아니다 — 다만 그
    종결값("완료")이 실제로는 오판이라는 뜻.

  둘 다 **"환불 확정"이라는 사건을 표현할 값이 애초에 없어서** 생기는 문제라 원인이 같고, 대체할
  값을 정하기 전에는 고칠 수 없다는 점도 같다. 두 후보: (1) 기존 "상품준비불가"를 Package
  도메인 보상조치에도 재사용(Item 도메인 취소와 같은 라벨로 통일) — 단 이 값은 원래 "이 상품
  자체를 준비할 수 없다"는 Item 도메인 어휘라 의미가 다소 늘어남, (2) "환불됨" 같은 새 값을
  신설해 취소와 명확히 구분. **두 필드를 따로 고치면 라벨이 서로 어긋날 위험이 있다** —
  `customer_facing_status`는 "환불됨"인데 `internal_order_status`는 "완료"로 남는 식의 불일치가
  생길 수 있으므로, 값을 정할 때 반드시 같이 검토한다. 실제로 문제가 되는 시점(알림agent 구현
  등)에 재검토.
- **포장agent가 창고처리agent와 달리 Sensor/Actuator 구분 없이 item_status만 직접 갱신한다.**
  창고처리agent(`warehouse.py`)는 `[Sensor]` 로그로 위치확인을 판단 이전 단계로 명시적으로
  분리해두는데, 포장agent(`packaging.py`)는 그런 구분 없이 조건 체크 후 바로 `item_status`를
...
- **주문검증agent가 `payment_status != "완료"`를 세 가지 다른 상황을 뭉뚱그려 전부 즉시
  검증 실패로 처리한다.** `order_validation_agent`는 self-loop 없는 1회성 관문이라 지금은
  구분할 방법이 없다. 실제로는 성격이 다른 세 상황이 섞여 있다:
  1. **결제 처리 중**(비동기 승인 대기) — 워크플로우 진입 자체를 미뤄야 하는 상황.
  2. **결제 실패이지만 재시도 가능**(한도초과 등) — 다른 결제수단으로 재시도하면 통과할 수
     있어야 하는 상황.
  3. **결제 실패로 완전 종결**(계정 차단 등) — 지금처럼 즉시 거부가 맞는 상황.

  지금은 셋 다 3번처럼 즉시 거부된다. 2번을 지원하려면 `order_validation_agent`도 다른
  self-loop 게이트들처럼 재진입 가능한 구조가 필요하거나, 결제 재시도를 별도 진입점(그래프
  재실행)으로 처리해야 한다.
- **추적agent(정확히는 mock_carrier_signal)가 같은 그래프 실행에서 봉인된 패키지들을
  lockstep으로 함께 전진시킨다.** 실제로는 캐리어마다, 패키지마다 이벤트 도착 시각이
  다르므로 서로 다른 배송지의 두 패키지가 같은 틱에 항상 나란히 진행 단계가 바뀌는 일은
  없다. 원칙6 위반은 아니다 — 패키지 간에 서로를 막는 의존성은 코드에 없고(먼저 끝난
  패키지는 이후 틱에서 조용히 스킵될 뿐), lockstep은 "봉인 시점 자체가 이 POC에서는
  항상 동시에 발생한다"는 상류 타이밍의 결과일 뿐 하류의 join/wait 구조가 아니다.
  온톨로지/실제 이벤트 연동 단계에서 패키지별 독립적인 이벤트 타이밍으로 대체해야 함.

- **포장대기게이트가 배송중게이트와 같은 order-wide 블로킹 패턴을 그대로 갖고 있다 —
  배송중게이트만 고쳐졌다.** `route_after_packaging_wait_gate`는 "주문에 속한 패키지
  중 하나라도 미봉인·미보상조치면 전부 재시도"라는 전역 판단이다(`route_after_in_transit_gate`가
  배송중 단계에서 갖고 있던 것과 동일한 패턴, "검토 후 현재 구조 유지로 확정" 참고). 한
  패키지(예: "합배송희망"으로 결정된 item이 딸린 패키지)가 재시도 한도를 소진해 보상조치될
  때까지, 이미 준비된 다른 배송지의 패키지도 포장 이후 단계로 전혀 넘어가지 못한다.
  배송중게이트는 7단계에서 "패키지별 개별 판단"으로 고쳐졌지만 포장대기게이트는 그
  수정 대상이 아니었다. **재현 시나리오 없음** — 기존 시나리오는 지연 item이
  "부분수령희망"으로 애초에 그룹핑에서 제외되거나(시나리오 9), 배송지가 하나뿐이라
  다른 패키지가 막히는 걸 보여줄 대상이 없다(시나리오 10). 서로 다른 두 배송지에
  (a) 지연 없는 item, (b) "합배송희망"(또는 기본값)으로 결정될 통관지연류 item을
  함께 넣으면 재현 가능할 것으로 보인다.
<a id="dead-fields"></a>
## 미구현/죽은 필드 종합
...
- **배송 중 파손(Package 단계) 미구현.** `ItemDelayReason`에는 "파손"이 있지만
  `_PACKAGE_DELAY_SIGNAL`(Package 단계 지연 카테고리)에는 "교통지연"/"자연재해"만 있고
  "파손"이 없다. 추가하더라도 기존 자연재해 로직을 그대로 재사용할 수 없다 —
  `mock_carrier_signal`은 보상조치(환불)된 패키지도 계속 전진시켜 결국 "배송완료"까지
  도달시키는데, 이는 자연재해(상품은 멀쩡할 수 있음)에는 맞지만 파손(상품이 이미 못 쓰게 됨)에는
  안 맞는다 — 파손이면 별도로 진행을 멈추는 새 분기가 필요하다. Item 도메인의 "취소됨"과 유사한
  종결 처리가 필요할 수 있음. 실제 배송 중 파손의 택배사-쇼핑몰 간 정산/청구 절차는 이 시스템의
  범위 밖(별도 백오피스 프로세스).
