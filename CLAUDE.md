...
DESIGN.md 각 섹션 끝의 "[JOURNAL.md](JOURNAL.md) 참고" 링크는, 그 결정의 배경/과정이 궁금하거나
재검토가 필요할 때만 실제로 열어본다. **매 세션 시작 시 JOURNAL.md를 통째로 읽지는 않는다.**
다만 다음 상황에서는 먼저 JOURNAL.md를 검색/확인한다:
1. 이미 시도했다가 폐기된 접근인지 확인이 필요할 때 (예: "~하면 어떨까" 제안이 나왔는데 익숙하게 느껴질 때)
2. DESIGN.md의 결론만 보고는 "왜 이렇게 됐는지" 설명이 부족하다고 판단될 때
3. 사용자가 명시적으로 "예전에 이거 논의했던 거 아니야?"라고 물을 때

## JOURNAL.md 기록 기준

JOURNAL.md는 git commit으로 복원되지 않는 것만 남긴다 — git diff는 "무엇이 바뀌었는가"만
보여주지, "왜 그 방향을 택했는지", "코드로 이어지지 않은 시도가 왜 실패했는지"는 보여주지
않는다. 그래서 다음만 기록한다:

- 코드에 반영되지 않고 폐기된 시도/가설 (git에 흔적이 안 남음)
- 여러 대안 중 하나를 택한 논거와 기각 이유
- 재현 절차, 그리고 재현으로 실제로 새롭게 드러난 사실
- 예상과 다르게 나온 결과(비결정론적 판단 등 특이 관측)

다음은 기록하지 않는다 (커밋 메시지/테스트 로그로 충분히 복원 가능):

- "pyright 0 errors", "N개 시나리오 전체 통과", "회귀 없음"처럼 아무 새로운 정보 없이
  통과만 확인하는 문장 — 특이사항이 있을 때만 그 특이사항을 적는다.
- 이미 DESIGN.md/커밋 메시지에 있는 결론의 단순 재진술

매 세션 시작 시 이 문서를 통째로 읽지 않는다 — DESIGN.md의 "JOURNAL.md 참고" 링크를 따라
필요할 때만 부분적으로 연다(위 "작업 전 필수" 섹션 참고).
