...
DESIGN.md에는 노드 목록, State 스키마, 단계별 실행 결과, 그리고
...
검증 자체는 남기되(결과가 특이하거나 판단 근거가 되는 경우), 검증의 세부 수치·절차·중간
관측값은 결론만 남기고 생략한다. "간격이 10초로 맞춰졌다"면 충분하고, "1차 2.6초, 2차
7.4초"처럼 매 관측치를 나열하지 않는다.
...
버전 번호, 개수, 필드 목록 등 "바뀔 수 있는 값"은 대표 문서(주로 DESIGN.md)의 한 곳에만
두고, 다른 문서/파일에서는 그 값을 다시 적지 않고 해당 문서를 참조하도록 서술한다.
값을 중복해서 적으면 한쪽만 갱신되고 다른 쪽이 밀리는 문제가 반복된다(예: CLAUDE.md의
"State 스키마(v12)"가 DESIGN.md는 v16까지 갱신된 뒤에도 안 고쳐진 채 남아있었음).

예외 — 각 문서의 목적상 그 자리에 내용이 그대로 있어야 하는 것들은 링크로 대체하지
않고 중복을 허용한다. 다만 하나를 고치면 반드시 나머지도 같이 검토한다:

- 핵심 설계 원칙 6가지 — DESIGN.md("핵심 설계 원칙")/README.md("핵심 설계 원칙")/
  CLAUDE.md("State 설계 원칙") 세 곳
- 노드 목록 표 — DESIGN.md("노드 목록 (최종)")/README.md(아키텍처 다이어그램 아래 표)
  두 곳

위 항목을 수정할 때는, 수정 후 다른 문서의 같은 항목도 grep으로 찾아 함께 갱신했는지
확인한다.
